...
<script>
	...
	export default {
		...
		methods: {
			...
			phoneValidator(mobile, callback) {
				if (mobile == "") {
					callback("手机号不能为空");
					this.logining = false;
					return
			...
			async toLogin() {
				...
				var postdata = {
					token: '',
					loginMark: "Customer_" + mobile, //learun.deviceId(),// 正式请换用设备号
				...
				this.$httpRequest(opts, postdata).then(res => {
					...
					if (res.data.code == 200) {	
						var logininfo = {
							account: mobile,
							token: res.data.data.baseinfo.token,
							date: that.date_time(),
							loginMark: "Customer_" + mobile
						};
						uni.setStorage({
						    key: 'logininfo',
						    data: logininfo,
